...
export default function CandidatesPage() {
    ...
    // Sync modal filters with active filters
    useEffect(() => {
        setModalStatusFilter(statusOptions.find(s => s.value === statusFilter) || {label: "All", value: "all"});
    ...
    const handleResetFilters = () => {
        // Find default status from statusOptions
    ...
    if (isError) return <p className="text-red-600">Something is wrong....</p>;
    ...
    return (
            <div className="p-4 space-y-6 flex flex-col">
                ...
                {/* Status filter badges */}
                <div
                    className="flex justify-center gap-4 bg-white p-4 rounded-lg shadow dark:bg-gray-800 max-w-[500px] mx-auto">
                    {statusOptions.map((status) => (
                        <Button
                            outline
                            key={status.value}
                            color={status.color}
                            className={statusFilter === status.value ? "ring-2 ring-offset-1" : ""}
                            onClick={() => {
